...
| 10 | Monitor | Correlation (matrix + rolling, on returns), risk-regime composite (8-component z-score with attribution), sortable grid of all 58 indicators with history (1d/5d/1M change, 1-year percentile, sparkline), conditional forward-return study |
| 11 | Positioning | Consolidated: breadth + % above 200MA, put/call + SKEW, CFTC COT (metals + energy), 13F institutional changes |
...
- **Append-only CSVs:** `extract_historical_data.py` uses `append_to_csv()` which deduplicates by timestamp column. Never overwrites. Since 2026-09-16 every call holds a per-file `fcntl` lock (`historical_data/.<file>.lock`) and writes atomically (temp file + `os.replace`) — `macro2-ibkr-stream`, `fast_extract` and the full extraction rewrite the same files concurrently, and an unlocked overlap once left a blank-timestamp row at the end of `gold.csv`. Timestamps are parsed as ISO 8601 (never an inferred format); unparseable ones are dropped with a warning. Daily writers pass `replace_daily_dates=True` so a bar re-fetched under a different timezone convention replaces the old copy; NaN observations are not written.
- **`date` column conventions:** daily bars carry the exchange-local date (futures = New York, FX = London — the IBKR stream matches this per contract type). Survey series (`aaii_sentiment.csv`) are dated by the survey week-ending, not the collection day.
...
| S&P 500 forward P/E / forward EPS | multpl.com (via `_sp500_multiples_fallback`), yfinance SPY | `65_sp500_multiples.forward_pe` is `100 / multpl earnings yield`, and that yield is *trailing* — the "forward" P/E equals the trailing P/E (25.91 vs 25.92 on 2026-09-16), so the forward ERP is really trailing. SPY's yfinance `info` has no forward fields at all | Treat as trailing. `sp500_fundamentals.csv` leaves `pe_ratio_forward` / `forward_eps` / `forward_earnings_yield` empty on purpose (trailing EPS is derived as price ÷ trailing P/E) |
| AAII Sentiment | aaii.com scrape | ~~Regex matched the all-time-records table (75.0 / 70.3) and the voting widget~~ **FIXED 2026-09-16** — parses the `.ssv2-gauge` current-week block, validates the shares, dates rows by survey week. The history spreadsheet is behind Imperva bot protection for scripted clients | History backfill needs a manually downloaded `sentiment.xls` |
...
- **Analytics:** statsmodels (Engle-Granger cointegration in `react_dashboard/backend/analytics.py`) — in `requirements.txt`, so it must be present in **both** VPS venvs
...
### 2026-09-09
- **Added: phase 3 pairs — `/api/analytics/pairs`** — hedge-ratio spread (OLS of log a on log b), Engle-Granger cointegration via `statsmodels.tsa.stattools.coint` (new dependency in `requirements.txt`, installed in BOTH VPS venvs — dry-run confirmed it touches nothing pinned), AR(1) half-life, and z-score against the trailing year plus the plain ratio. Ordering is deliberate: stationarity is reported first because a z-score on a non-stationary spread is a position on a random walk, not a reversion signal — and every classic pair probed (RTY/ES p=0.16, gold/silver p=0.28, BTC/ETH p=0.78) comes back *not* cointegrated over 5 years. The panel says so rather than showing ±2σ bands as if they meant something.
- **Added: phase 4 correlation-regime scan — `/api/analytics/corr-regime`** — 14 cross-asset pairs ranked by how far the current rolling correlation sits from its own trailing-year distribution (|z|≥2 break, ≥1.5 watch). Memoized per window on the cache mtime. First live scan: **ES~BTC decoupling (−3.0σ)** and **gold~BTC coupling (+2.8σ)**, ES~HY OAS and ES~VIX on watch. This is the dashboard surface for the alert; Telegram push was deliberately left out per the earlier scope decision and is a small addition on the QA agent's existing sender.

- **Fixed: the macro calendar's FRED release-id map was wrong for 9 of 10 entries** — `macro_calendar_extractor.py` mapped 11→"Employment Situation" (actually the Employment *Cost* Index, quarterly), 50→"GDP" (actually Employment Situation), 53→"PCE" (actually GDP), 21→"PPI" (H.6 money stock), 200→"Retail Sales" (CBOE statistics), 197→"Consumer Sentiment" (Dow Jones Averages), 175→"JOLTS", 18→"Industrial Production", 51→"Housing Starts" — only CPI=10 was right. Every calendar row except CPI was therefore a projection off the wrong series' last print. Caught when NFP returned 20 dates in five years. Corrected by name lookup against `/fred/release` (NFP 50, GDP 53, PPI 46, PCE 54, JOLTS 192, Retail Sales 9, IP 13, Housing Starts 27); Consumer Sentiment dropped — UMich is not a FRED release. **Rule: verify a FRED release id by name via the API before using it; the ids do not follow any pattern.**
- **Changed: the calendar now uses FRED's published schedule instead of cadence projection** — `/release/dates` returns scheduled future dates with `include_release_dates_with_no_data=true`; the old "+30 days from the last print" produced Housing Starts on July 4th. Projection survives only as a fallback when FRED has nothing scheduled. `refresh_calendar()` now regenerates the CSV (last year of actual dates + scheduled window) rather than merging with stale projected history, and writes **`historical_data/macro_event_history.json`** — five years of actual dates per event (FOMC from the `fomcminutes<YYYYMMDD>.htm` links on the Fed calendars page; FRED release 101 "FOMC Press Release" is unusable — it emits a date on the 1st of every month).
- **Added: phase 2 pair analytics — `/api/analytics/beta`, `/api/analytics/lead-lag`** — β(a on b) = cov/var(b) with the reverse, R², and rolling β (RTY/ES 1.12 at R² 0.73; gold/DXY −1.02 at R² 0.15, currently −2.67 = −2.2σ). Lead/lag is corr(a_t, b_{t+k}) for k∈[−10,10] with a ±2/√n noise band; the verdict says "no lead beyond noise" unless the off-zero peak beats lag-0 by more than the band — which on daily data is the usual answer, and the honest one.
- **Added: event study — `/api/analytics/events`, `/api/analytics/event-study`** — average cumulative path and event-day distribution of any series around every past occurrence of a catalyst, against a normal-day baseline. The baseline is the finding: CPI moves gold **0.99×** a normal day (not a catalyst for gold), FOMC moves ES **1.34×**, NFP moves DXY **1.37×**, CPI moves BTC **1.31×**. Unconditional on the surprise, so means are muted while absolute moves carry the signal; the payload says so. Rendered in Tab 10 with a mean/median path chart and the last eight events.

- **Added: series resolver + correlation (`/api/series/catalog`, `/api/analytics/correlation`)** — a shared access layer resolving any instrument to a clean daily series from three shapes: `indicators[key]['historical']`, `indicators[key]['historical_<sub>']` (sector ETFs, crypto), and Hyperliquid CSV columns resampled to daily. 83 correlatable series. Correlation is computed on **log returns, never price levels** (two assets that merely both trend score near 1.0 with no co-movement), falling back to first differences for series that cross zero (spreads, real yields). Every pair reports its own overlap count — two series can share the catalog and barely share a calendar — and pairs under 60 overlapping days are flagged `thin`. Rolling correlation reports its z-score against its own history, which is where the tradeable signal lives: gold/BTC is +0.105 over 1,254 days but **+0.586 on the 60-day window, +2.9σ** from its own norm.
- **Fixed: BTC had no usable history for cross-asset work** — `84_hl_perps.btc` holds scalars only, and the Hyperliquid CSV starts 2026-03 (115 days overlapping gold). New `get_crypto_majors()` in `yfinance_extractors.py` registered as **`87_crypto_majors`** supplies BTC/ETH/SOL 5-year daily closes, taking gold∩BTC overlap from 115 → **1,254 days**. The resolver deliberately prefers it over the Hyperliquid CSV fallback.

- **Added: chart drawer + TradingView engine** — expanded k-lines were a 350 px inline strip; there is now a full-width 82vh drawer (Esc to close), opened by a ⤢ button on any OHLCV expander or from the ⌘K palette. Two engines behind a toggle: TradingView's free Advanced Chart widget (indicators, drawing tools) for listed instruments, and the native lightweight-charts engine over `/api/intraday` / `/api/hl/ohlcv` for repo-only data. The embed is `React.lazy`'d into its own chunk — main bundle went 135 → 140 kB gz, first paint unaffected. **Free-widget licensing gotcha:** `COMEX:GC1!` / `CME_MINI:ES1!` resolve in TradingView symbol search but render "only available on TradingView" in the free widget — exchange futures data is not licensed for it. Futures map to free proxies (`TVC:GOLD`, `SP:SPX`, `AMEX:IWM`, `AMEX:CPER`, …) and `config/instruments.js` carries a `proxyNote` the drawer displays, so a proxy is never shown as if it were the contract.
- **Fixed: `package.json` never declared `lightweight-charts`** — it was installed but undeclared, so a clean `npm install` could not build the frontend. Pinned `^4.2.2`.
- **Added: macro catalyst calendar strip** — `historical_data/macro_catalyst_calendar.csv` was built for the options-strategy downstream but never surfaced, and **nothing rebuilt it** (stale since May). `refresh_calendar()` now runs at the end of `scheduled_extract.py`, and `/api/calendar` feeds a header strip showing the next catalysts with countdowns. CLAUDE.md had documented `build_macro_calendar()`, which never existed — corrected to `refresh_calendar()`.
- **Added: `react_dashboard/backend/analytics.py`** with four endpoints — `/api/monitor` (58-row grid: latest, 1d/5d/21d change, 1-year percentile, sparkline), `/api/analytics/regime` (risk-on/off composite from 8 sign-adjusted z-scores, with per-component attribution and top 1-day movers), `/api/analytics/forward-returns` (conditional forward-return study reported **against the unconditional baseline** over the same sample, with the overlapping-window caveat in the payload), and `/api/positioning/13f`. Monitor and regime are memoized on the cache file's mtime, same contract as `_serialized_snapshot`.
- **Added: ⌘K command palette** — jump to any tab or open any instrument's large chart.

### 2026-08-31
- **Perf follow-ups: polling diet + HTTPS/HTTP2** — (1) the 60 s auto-refresh re-downloaded the full 1.5 MB gz payload per client per tick; now each tick fetches lite (~57 KB) and deep-merges it over current state (`mergePreserving` keeps previously-loaded series so open charts survive), with a full payload every 5th tick — ~4x less transfer/CPU at the same scalar freshness. Verified live: lite polls at t=61/121/181/241 s, charts intact after four merge cycles. (2) awehawk.cloud now serves HTTPS (Let's Encrypt via certbot --nginx, cert to 2026-11-29, `certbot.timer` renews; dry-run hit a transient LE "service busy" rate limit — retry later, not a config issue) with `http2 on;` and a 301 from HTTP. `server_name _` → explicit `awehawk.cloud www.awehawk.cloud`. wss:// live feed verified through the TLS proxy (`useHyperliquidWS` was already protocol-aware). Sibling sites (wc.awehawk.cloud, /poly/, /btc/) unaffected. No repo code referenced http:// URLs, so the redirect breaks nothing.

- **Perf: first visit cut from ~5–8 s to ~1.5 s (React dashboard)** — four layers were slow for first-time visitors: (1) the JS bundle was 5.3 MB/1.6 MB gz because `plotly.js-dist-min` was statically imported — now behind `React.lazy` via `components/LazyPlot.jsx` (all charts are `{open && <Plot/>}`-gated, so plotly's 1.47 MB gz chunk loads only on first chart expand); main bundle → **424 KB / 134 KB gz**. (2) `/api/indicators` re-serialized ~9 MB of JSON per request (~2 s ttfb) — now memoized on the cache file's mtime (`_serialized_snapshot`), with the IBKR overlay made copy-on-write so it can't poison the memo. (3) The full payload shipped 5-year series for charts that are all collapsed — new `?lite=1` strips serialized pandas objects at any depth (9.8 MB → **155 KB / 36 KB gz**); `App.jsx` fetches lite first (cards paint immediately), then full in the background. (4) nginx served assets with no cache headers and runtime gzip — content-hashed `/assets/` now `immutable` + `gzip_static` (`.gz` precompressed at build), and `index.html` is `no-cache` (heuristic caching would leave returning visitors pointing at deleted hashed bundles after a `--delete` deploy → blank page). nginx gotcha: `add_header` in a location drops inherited server-level headers, so the security headers are restated in both new locations.
