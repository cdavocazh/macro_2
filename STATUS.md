...
| VPS: IBKR stream | systemd macro2-ibkr-stream.service | 2026-09-21 | Long-running daemon, ib_async, 3s JSON snapshots. Streams continuously only since the 2026-09-21 fix (before, ticks arrived only at ~30-min reconnects). 16 instruments (2YY retired); VIX = IB Error 354, no CBOE subscription. Restart safely with `systemctl stop … && sleep 60 && systemctl start …` (RestartSec=3600 hides a crash for an hour) |
...
- **Feed-collector sweep (2026-09-21, all deployed to the VPS)** — commits `46ac6de` (ADP: monthly `adp_employment.csv` + new `adp_employment_weekly.csv`; `earnings_calendar.csv` is now a snapshot log `timestamp,date,symbol,report_date,source`), `a7edc28` (IBKR stream pumps its connection on its own thread, rolls front months hourly, 2YY retired — use `us_2y_yield.csv`), `cb00e56` (Hyperliquid: LINK/DOGE/AVAX/SUI restored, builder perps on live xyz markets, price backfill applied `--fill first` with provenance in `historical_data/.hl_perps.provenance.json`), `5e68667` (writer-declared column contracts `historical_data/.<stem>.columns.json`, read by the CC feed scanner), `f94288d` (history contract guard for the CC execution builder). Details in QA_SOP Bug Log. **Still open:** a Hyperliquid repair pass (5,626 frozen builder-quote rows 08-30→09-21, pre-08-30 funding/OI units, GMT+8 rows before 2026-03-24, wrong-market spot history) is in progress; **decisions pending with the user:** the remaining HL spot-stock pairs (TSLA/AMZN/META/QQQ/SPY trade ~$0–8k/month: retire, switch to xStock pairs, or use stock perps) and `vix_ibkr` (subscribe to CBOE index data, use delayed data, or drop VIX).
- **Consensus, instruments, Hyperliquid repair (2026-09-22, deployed)** — `a9195b7` consensus + surprises (ForexFactory + Kalshi; first ForexFactory-scored release: jobless claims 2026-09-24), `afadcb7` VX/IEF/ZN execution history, `48dd5d6` Hyperliquid contracts + relay registry + data repair (applied 00:39 UTC; `--rollback` available), `e3f6bda` contract key/active_since. Feed scanner: 1 flag of 142 — `hl_spot_stocks` AMZN/QQQ have never held a correct value; the remaining HL spot pairs (TSLA/AMZN/META/SPY/QQQ, ~$0–8k/month) await the user's keep / xStock / stock-perp decision. Known data defect: `cot_silver.csv` carries rows from a different contract (CC's factor code skips them). `ism_pmi.csv` is not the ISM PMI (task chip open).
...
- **`macro-extract` timeout is 900 s (drop-in `timeout.conf`, 2026-09-19)** — the unit file still says `TimeoutStartSec=600`, but successful full extractions take 5–10 min (median 8.3), so 600 s killed ~1 run in 5 part-way through the historical-CSV phase. A killed run leaves no partial CSVs (writes are atomic since v2.9.1) but skips the rest of that run and its `repair_cache_errors.py` post-step. If runs creep toward 15 min, look at the network-bound steps (Top-20 financials, OpenBB) before raising it again.
