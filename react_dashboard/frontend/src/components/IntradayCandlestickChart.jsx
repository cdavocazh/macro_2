...
// GMT+8 offset in seconds
const GMT8_OFFSET = 8 * 3600;

/**
 * OHLCV candlestick chart for yfinance instruments using TradingView lightweight-charts.
 * Lazy-loaded: chart is only created and data fetched when the user expands the <details>.
 */
export default function IntradayCandlestickChart({ instrumentKey, label, color = '#1f77b4' }) {
  ...
  const fetchData = useCallback(async (intv) => {
    ...
    try {
      const resp = await api.get(`/intraday/${instrumentKey}`, { params: { interval: intv } });
