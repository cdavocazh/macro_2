...
| 2026-09-21 | The IBKR streamer wrote stale prices into 7 per-instrument CSVs and the summary — ~6 rows in 7 were stale repeats, and `micro_2y_yield` wrote 4.182 for days | IB serves the last trade print indefinitely once a subscription goes quiet, and `last_update` is bumped by a tick of ANY field, so nothing recorded when the PRICE last moved. **Corrected later the same day:** for liquid instruments the repeats came from the daemon's dead event-loop pump (next rows); the quiet-subscription explanation holds only for genuinely dead contracts such as 2YY, and `micro_2y_yield`'s 4.182 was 217 rows on 2026-09-09 only, an off-market print (DGS2 4.43) | `StreamingQuote.last_price_time` is stamped only where `last` changes; both write paths skip an instrument whose price is older than 3 × `IBKR_CSV_INTERVAL` and log the skip. These files now show gaps during dead subscriptions — missing beats wrong | `data_extractors/ibkr_streaming.py`, `ibkr_fast_extract.py` |
...
| 2026-09-21 | `adp_employment.csv` read as a weekly series gone stale ("stale 51d") | 82e651f (2026-03-09) switched `get_adp_employment` from weekly ADPWNUSNERSA to monthly ADPMNUSNERSA without migrating the file; `append_to_csv` merged the 200 monthly rows into 810 weekly ones (27 Saturday-the-1st weeks overwritten). Values matched against FRED | `adp_employment.csv` is monthly-only and every row carries `series_id`; the weekly series moved to `adp_employment_weekly.csv`. `_extract_simple_series(series_id=, cadence=)` + `append_to_csv(guard=)` refuse a foreign series, off-grid dates, or an unlabelled file. Repair: `scripts/repair_adp_earnings_20260921.py` (`--rollback` available) | `extract_historical_data.py`, `data_extractors/fred_extractors.py` |
| 2026-09-21 | `earnings_calendar.csv`: 6,720 rows holding 1,470 facts; 1,036 conflicting repeat-dates, 671 out-of-order | The writer stored `date` = REPORT date and no timestamp, so `append_to_csv` skipped de-duplication and appended every run verbatim | Now a point-in-time snapshot log `timestamp,date,symbol,report_date,source`, keyed (as-of day, symbol), last run of the day wins; refuses any other schema. A snapshot log, not latest-date-per-report, so as-of reads cannot leak later confirmations | `extract_historical_data.py` |
| 2026-09-21 | The IBKR stream never streamed: ticks arrived only at each ~30-min stale-reconnect (84% of ES rows in US hours repeated the previous row; ES changed every 30.65 min median) | `ibkr_fast_extract` pumped `ib.sleep()` from a daemon thread; ib_async's `getLoop` gives that thread a new socket-less loop, so the socket was read only while the main thread ran its own loop. `last_price_time` was also stamped on any ticker update, not only trades | Pump on the connecting thread; manifest swaps inline; `last_price_time` only when `last` changes or volume grows; hourly `_roll_front_months` (per symbol, 10 s qualify cap, tolerant of ib_async 2.x `[None]`) because the reconnect no longer rolls contracts as a side effect; Error 101 (market-data line cap) re-requested with 60 s–15 min backoff. **2YY retired** from `INSTRUMENTS`: a price on 5 of 132 days, prints ~25 bp off DGS2, and it held a line while Error 101 blanked USDJPY — use `us_2y_yield.csv` | `ibkr_fast_extract.py`, `data_extractors/ibkr_streaming.py`, `agent/shared/freshness_sla.py` |
| 2026-09-21 | `hl_perps.csv`: 31 dead columns (XRP/LINK/DOGE/AVAX/SUI since 03-19, premium since 03-21, Brent since 03-22) and 4 frozen ones (oil 76.4 / sp500 7435.0 / natgas 3.2429 / copper 6.33 for 5,626 rows since 08-30) | A pre-git registry edit on 03-19 swapped five alt perps out of `HL_PERPS`; the header kept their columns. 8a50610 moved the builder perps to flx listings dead since June, and the writer ignored the extractor's own `illiquid` flag. The scanner printed only 3 blank columns and skipped intraday files for frozen values | Registry restored; builder perps on live xyz markets; illiquid quotes withheld; premium written; context-derived fields only when the context loaded; stdout flushed before `os._exit`. `scripts/backfill_hl_perps_20260921.py --fill first` filled 55,758 price cells (funding/OI have no faithful history), provenance in `.hl_perps.provenance.json`. Scanner lists every blank column and has frozen-column / repeat-rate / stopped-moving intraday checks | `hl_extract.py`, `data_extractors/hyperliquid_extractor.py` |
| 2026-09-21 | A blank column could not say whether it was retired or broken | The scanner had only a per-file `KNOWN_EMPTY_COLUMNS` exception list; no writer stated which columns it owns | `extract_historical_data.declare_columns()` writes `historical_data/.<stem>.columns.json` (active / retired with since+reason / unavailable with reason); the CC scanner flags orphan columns, retired columns written again, and stale contracts. Wired: IBKR daemon (9 files), sp500_fundamentals | `extract_historical_data.py`, `ibkr_fast_extract.py`, `data_extractors/ibkr_streaming.py` |
| 2026-09-22 | No consensus expectations anywhere in historical_data (blocked the CC surprise layer and post-event initiation) | — (never collected) | `data_extractors/consensus_extractors.py`: ForexFactory weekly consensus + Kalshi market-implied medians snapshotted into `macro_consensus.csv`; `macro_surprises.csv` scores each release against its FRED first-release actual (equal to Kalshi's settlement 508/511 times since 2021; the 3 misses are Kalshi-side, 2025 shutdown). Kalshi history back-filled from settled-market candles (416 releases since 2021-09). Wired into the full extract | `data_extractors/consensus_extractors.py`, `extract_historical_data.py` |
| 2026-09-22 | VIX / UST10Y views scored on series that cannot be traded as modelled | — | `futures_history_extractors.py`: CBOE VX curve (2014→) and a FORWARD-adjusted VX front series (no roll ever rewrites history; adjusted levels go negative by design — use raw = price − roll_adjust_cum for levels), IEF (recommended 10Y instrument, duration 7.10, R² 0.93) and ZN=F; append-only, wired into the full extract | `data_extractors/futures_history_extractors.py` |
| 2026-09-22 | Hyperliquid history: 5,719 frozen builder quotes per coin, funding/OI in two unit conventions, Mac GMT+8 rows, a fake-zero row, wrong-market spot history; dashboard relay on its own stale registries | See the 2026-09-21 hl_perps row; plus 8a50610 changed funding (×3×365 → ×24×365) and OI (coins → USD) mid-column | `scripts/repair_hl_20260921.py` applied on the VPS 2026-09-22 00:39 UTC (each repair proven first: funding ×8 from fundingHistory ratio 0.1251; OI ratio ~1.000 across the break; timezone by BTC-candle fit 1.0; provenance recipe unchanged); records in `historical_data/.hl_*.repairs.json`, `--rollback` available. hl_extract declares its column contract; `hl_ws_service` imports the extractor's registries (macro-react restarted) | `hl_extract.py`, `data_extractors/hyperliquid_extractor.py`, `react_dashboard/backend/hl_ws_service.py` |
| 2026-09-22 | Scanner read keyed snapshot logs as conflicting repeat-dates, and new columns as "blank since never" | Only single-column panel keys were detected; no notion of when a column started | `declare_columns(key=…, active_since=…)`: the writer states its row key and the day a column began being written; the CC scanner checks conflicts on the declared key | `extract_historical_data.py` |
